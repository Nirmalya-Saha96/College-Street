...
import live.nirmalyasaha.me.bookstore.activities.ui.activities.CartListActivity
...
open class CartListAdapter(private val context: Context, private var list: ArrayList<Cart>, private val updateCartItems: Boolean): RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        if(holder is MyViewHolder){
            GlideLoader(context).loadBookImage(model.image, holder.itemView.iv_cart_item_image)
            holder.itemView.tv_cart_item_title.text = model.title
            holder.itemView.tv_cart_item_author.text = model.author
            ...
            holder.itemView.ib_delete_cart_item.setOnClickListener {
                ...
                FirestoreClass().deleteCartItem(context, model.id)
            }
            ...
            holder.itemView.ib_remove_cart_item.setOnClickListener {
                if(model.cart_quantity == "1"){
                    ...
                    cartHashMap[Constants.CART_QUANTITY] = (cartQuantity - 1).toString()
                    if (context is CartListActivity) {
                        context.showProgressDialog(context.resources.getString(R.string.please_wait))
                    }
                    FirestoreClass().updateCart(context, model.id, cartHashMap)
                }
            }
